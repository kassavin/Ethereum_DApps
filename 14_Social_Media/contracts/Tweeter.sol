...
contract Tweeter {
    ...
    event TweetSent ( uint id, address indexed author, string content, uint createdAt );
    event MessageSent ( uint id, string content, address indexed from, address indexed to, uint createdAt );
    ...
    struct Tweet { uint id; address authorAddress; string authorName; string content; uint createdAt; }
    struct Message { uint id; string content; address from; address to; uint createdAt; }

    modifier canOperate(address _from) {
        require( operators[_from][msg.sender] == true || msg.sender == _from, 'Operator not authorized');
        _;
    }
    ...
    function reverseResolve(address _authorAddress) view public returns(string memory) {
          return 'author name';
    }
    ...
    function getLatestTweets(uint count) view external returns(Tweet[] memory) {
        ...
        for (uint i = nextTweetId - count; i < nextTweetId; i++) {
            ...
            _tweets[j] = Tweet( _tweet.id, _tweet.authorAddress, _tweet.authorName, _tweet.content,  _tweet.createdAt);
            j += 1;
        }
    ...
    function _tweet( address _authorAddress, string memory _content) internal canOperate(_authorAddress) {
        
        string memory authorName = reverseResolve(_authorAddress);
        tweets[nextTweetId] = Tweet( nextTweetId, _authorAddress, authorName, _content, now);
    ...
    function _sendMessage( string memory _content, address _from, address _to) internal canOperate(_from) {
        ...
        conversations[conversationId].push(Message(nextMessageId, _content, _from, _to, now));
