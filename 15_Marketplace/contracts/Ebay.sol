...
contract Ebay {
    ...
    struct Auction { uint id; address payable seller; string name; string description; uint min; uint end; uint bestOfferId; uint bestOffer; uint[] offerIds; 
    }
    
    struct Offer { uint id; uint auctionId; address payable buyer; uint price;
    }
    ...
    function createAuction(string calldata _name, string calldata _description, uint _min, uint _duration) external {
        ...
        auctions[nextAuctionId] = Auction( nextAuctionId, msg.sender, _name, _description, _min, now + _duration, 0, 0, offerIds);
